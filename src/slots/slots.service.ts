...
export class SlotsService {
  ...
  listAvailable(eventTypeId: string, from?: string, to?: string): Slot[] {
    ...
    return this.store
      .listSlots()
      .filter((slot) => slot.durationMinutes === eventType.durationMinutes)
      .filter((slot) => {
        if (fromDate) {
          const slotStart = new Date(slot.startAt)
          if (slotStart.getTime() < fromDate.getTime()) return false
        }
        if (toDate) {
          const slotStart = new Date(slot.startAt)
          if (slotStart.getTime() > toDate.getTime()) return false
        }
        return true
      })
      .filter((slot) => {
